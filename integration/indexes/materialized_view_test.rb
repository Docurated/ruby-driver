...
class MaterializedViewTest < IntegrationTestCase
  ...
  def test_can_retrieve_materialized_view_metadata
    ...
    assert_equal 6, mv_meta.primary_key.size
    assert_equal 'game', mv_meta.primary_key[0].name
    assert_equal :text, mv_meta.primary_key[0].type.kind
    assert_equal 'year', mv_meta.primary_key[1].name
    assert_equal :int, mv_meta.primary_key[1].type.kind
    assert_equal 'month', mv_meta.primary_key[2].name
    assert_equal :int, mv_meta.primary_key[2].type.kind
    assert_equal 'score', mv_meta.primary_key[3].name
    assert_equal :int, mv_meta.primary_key[3].type.kind
    assert_equal 'user', mv_meta.primary_key[4].name
    assert_equal :text, mv_meta.primary_key[4].type.kind
    assert_equal 'day', mv_meta.primary_key[5].name
    assert_equal :int, mv_meta.primary_key[5].type.kind

    assert_equal 3, mv_meta.partition_key.size
    assert_equal 'game', mv_meta.partition_key[0].name
    assert_equal :text, mv_meta.partition_key[0].type.kind
    assert_equal 'year', mv_meta.partition_key[1].name
    assert_equal :int, mv_meta.partition_key[1].type.kind
    assert_equal 'month', mv_meta.partition_key[2].name
    assert_equal :int, mv_meta.partition_key[2].type.kind

    assert_equal 3, mv_meta.clustering_columns.size
    assert_equal 'score', mv_meta.clustering_columns[0].name
    assert_equal :int, mv_meta.clustering_columns[0].type.kind
    assert_equal 'user', mv_meta.clustering_columns[1].name
    assert_equal :text, mv_meta.clustering_columns[1].type.kind
    assert_equal 'day', mv_meta.clustering_columns[2].name
    assert_equal :int, mv_meta.clustering_columns[2].type.kind
  ...
  def test_can_retrieve_quoted_mv_metadata
    ...
    assert_equal 2, mv_meta.primary_key.size
    assert_equal 'theKey', mv_meta.primary_key[0].name
    assert_equal :int, mv_meta.primary_key[0].type.kind
    assert_equal 'the;Clustering', mv_meta.primary_key[1].name
    assert_equal :int, mv_meta.primary_key[1].type.kind

    assert_equal 1, mv_meta.partition_key.size
    assert_equal 'theKey', mv_meta.partition_key[0].name
    assert_equal :int, mv_meta.partition_key[0].type.kind
    assert_equal 1, mv_meta.clustering_columns.size
    assert_equal 'the;Clustering', mv_meta.clustering_columns[0].name
    assert_equal :int, mv_meta.clustering_columns[0].type.kind
